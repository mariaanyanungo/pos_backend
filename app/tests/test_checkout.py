...
from app.services.payment_gateway import GatewayError, GatewayResult, SimulatedCardGateway
...
def test_cash_checkout_completes_sale_deducts_stock_and_issues_receipt(
    ...
    product, sale_id, cart = _cart(pos, make_product, quantity=2, price="10.00", tax_rate="20.00", stock_quantity=10)
    ...
    assert (D(payment["amount"]), D(payment["amount_tendered"]), D(payment["change_due"])) == (
        D("24.00"),
        D("30.00"),
        D("6.00"),
    )
    ...
    assert (D(receipt["subtotal"]), D(receipt["vat"]), D(receipt["total_amount"])) == (D("20.00"), D("4.00"), D("24.00"))
    assert (D(receipt["amount_tendered"]), D(receipt["change_due"])) == (D("30.00"), D("6.00"))
    assert receipt["sale_id"] == sale_id and receipt["payment_id"] == payment["payment_id"]

    assert get_stock(client, admin_headers, product["product_id"]) == 8
    ledger = client.get(f"/products/{product['product_id']}/stock-movements", headers=admin_headers).json()
...
def test_exact_cash_has_no_change(client, pos, make_product):
    _, sale_id, cart = _cart(pos, make_product, quantity=1, price="10.00", tax_rate="0.00")
...
def test_card_checkout(client, pos, admin_headers, make_product):
    product, sale_id, cart = _cart(pos, make_product, quantity=1, price="10.00", tax_rate="20.00")
...
def test_receipt_and_payment_are_readable_after_checkout(client, pos, make_product):
    ...
    by_id = client.get(f"/receipts/{body['receipt']['receipt_id']}", headers=pos.headers)
    ...
    assert client.get(f"/payments/{body['payment']['payment_id']}", headers=pos.headers).status_code == 200
...
def test_insufficient_cash_is_rejected_and_nothing_changes(
    ...
    product, sale_id, cart = _cart(pos, make_product, quantity=1, price="10.00", tax_rate="0.00", stock_quantity=5)
...
def test_idempotency_key_header_is_required(client, pos, make_product):
    _, sale_id, _ = _cart(pos, make_product)
    response = client.post(f"/sales/{sale_id}/checkout", json={"payment_method": "card"}, headers=pos.headers)
    assert response.status_code == 422


def test_checkout_of_unknown_sale_returns_404(client, pos):
    assert pos.checkout("00000000-0000-0000-0000-000000000000", "card").status_code == 404


def test_cashier_cannot_check_out_someone_elses_sale(client, pos, other_cashier_headers, make_product):
    _, sale_id, _ = _cart(pos, make_product)
    response = pos.checkout(sale_id, "card", headers=other_cashier_headers)
    assert response.status_code == 404


def test_a_paid_sale_cannot_be_checked_out_again_with_a_new_key(client, pos, admin_headers, make_product):
    ...
    assert get_stock(client, admin_headers, product["product_id"]) == 8  # deducted once only
...
def test_product_deactivated_after_carting_blocks_checkout(client, pos, admin_headers, make_product):
    product, sale_id, _ = _cart(pos, make_product, stock_quantity=10)
...
def test_selling_the_last_unit_leaves_zero_stock(client, pos, admin_headers, make_product):
...
def test_replaying_the_same_key_returns_the_original_result_without_charging_twice(
    ...
    assert replay.json()["payment"]["payment_id"] == first.json()["payment"]["payment_id"]
...
def test_another_cashier_cannot_replay_a_key_to_read_the_result(client, pos, other_cashier_headers, make_product):
    _, sale_id, _ = _cart(pos, make_product)
    pos.checkout(sale_id, "card", key="secret-key")
    response = pos.checkout(sale_id, "card", key="secret-key", headers=other_cashier_headers)
    assert response.status_code == 404


class DecliningGateway:
    def authorize(self, amount, idempotency_key):
        return GatewayResult(approved=False, failure_reason="Card declined: insufficient funds")
...
def test_declined_card_leaves_sale_open_stock_untouched_and_records_failed_payment(
    ...
    assert client.get(f"/receipts/by-sale/{sale_id}", headers=pos.headers).status_code == 404
    ...
    assert get_stock(client, admin_headers, product["product_id"]) == 8
    assert sorted(p["status"] for p in _payments(client, manager_headers, sale_id)) == ["captured", "failed"]
...
def test_failed_capture_voids_the_authorization_and_rolls_back(
    ...
    assert pos.get_sale(sale_id)["status"] == "open"
    assert get_stock(client, admin_headers, product["product_id"]) == 10
    assert [p["status"] for p in _payments(client, manager_headers, sale_id)] == ["failed"]
