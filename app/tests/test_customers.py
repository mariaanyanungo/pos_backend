...
def test_cashier_can_create_and_find_customers(client, auth_headers):
    ...
    assert (
        client.get(f"/customers/{customer_id}", headers=auth_headers).json()["name"]
        == "Jane Shopper"
    )
...
def test_duplicate_customer_email_returns_409(client, auth_headers):
    ...
    response = client.post(
        "/customers", json=_customer(name="Someone Else"), headers=auth_headers
    )
...
def test_customer_validation(client, auth_headers):
    assert (
        client.post(
            "/customers", json={"email": "jane@shoppers.com"}, headers=auth_headers
        ).status_code
        == 422
    )
    assert (
        client.post(
            "/customers", json=_customer(email="bad"), headers=auth_headers
        ).status_code
        == 422
    )
...
def test_update_customer(client, auth_headers):
    customer_id = client.post(
        "/customers", json=_customer(), headers=auth_headers
    ).json()["customer_id"]
    response = client.put(
        f"/customers/{customer_id}",
        json={"name": "Jane Q. Shopper"},
...
def test_only_managers_can_delete_customers(client, auth_headers, manager_headers):
    ...
    assert (
        client.delete(f"/customers/{customer_id}", headers=auth_headers).status_code
        == 403
    )
    assert (
        client.delete(f"/customers/{customer_id}", headers=manager_headers).status_code
        == 204
    )
