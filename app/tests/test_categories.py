def test_create_category(client, manager_headers):
    response = client.post(
        "/categories", json={"name": "Snacks"}, headers=manager_headers
    )
...
def test_duplicate_category_name_returns_409(client, manager_headers, category):
    response = client.post(
        "/categories", json={"name": category["name"]}, headers=manager_headers
    )
...
def test_create_category_without_name_returns_422(client, manager_headers):
    assert (
        client.post("/categories", json={}, headers=manager_headers).status_code == 422
    )
    assert (
        client.post(
            "/categories", json={"name": ""}, headers=manager_headers
        ).status_code
        == 422
    )
...
def test_cashier_can_read_but_not_write_categories(client, auth_headers, category):
    ...
    assert (
        client.post("/categories", json={"name": "X"}, headers=auth_headers).status_code
        == 403
    )
    assert (
        client.put(
...
def test_delete_category_is_soft(client, manager_headers, auth_headers):
    created = client.post(
        "/categories", json={"name": "Temp"}, headers=manager_headers
    ).json()
    assert (
        client.delete(
            f"/categories/{created['category_id']}", headers=manager_headers
        ).status_code
        == 204
    )
...
def test_cannot_delete_category_with_active_products(
    client, manager_headers, category, make_product
):
    make_product()
    response = client.delete(
        f"/categories/{category['category_id']}", headers=manager_headers
    )
    assert response.status_code == 409
...
def test_pagination(client, manager_headers):
    for index in range(5):
        client.post(
            "/categories", json={"name": f"Cat {index}"}, headers=manager_headers
        )
